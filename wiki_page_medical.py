...
def getcontent(title):
    ...
    open('./Rs_medical/%s' %title,'w+').write(snp_page)    # write into file
    
if __name__=='__main__':
    item = []
    i = 0
    if os.path.exists("./Rs_medical"): pass       
    else:
        os.mkdir("Rs_medical")
    file = open("SNPedia_Is_a_medical_condition.txt", "r")
    ...
    file.close
    item_id = item[100:423]   # choose part of SNP
    
    for name in item_id:
        if os.path.exists("./Rs_medical/%s"%name):
            continue        
        i += 1
        getcontent(name)
        print "%s loaded" %(name)

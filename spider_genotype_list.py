...
class PageScraper(object):
    ...
    def getTotal(self):
        try:
            html = self.getHTML(self.url)
            soup = BeautifulSoup(html, 'lxml')
            div_1= soup.body.find('div',attrs={'class':'mw-category-group'})
            div_2 = div_1.find_all('li')
            for link in div_2:
                self.makeOutput(link.a.text)        #get all href link in this section which of all are category item
        except AttributeError:
            return None   

    def makeOutput(self,snp_id):   
        a = time.strftime('%Y-%m-%d',time.localtime(time.time()))
        f = open("D:/tianguyuan/SNP_Genotype_Rs.txt", "a+")             #output to txt
        f.write("%s\n" %snp_id)    
        f.close


if __name__=='__main__':
    url = "https://www.snpedia.com/index.php?title=Category:Is_a_genotype&pagefrom=Rs9954153%28G%3BT%29#mw-pages"
    startTime = time.time()
